...
class MediaMixer:
    ...
    DEFAULT_VIDEO_DIR = Path(get_mp4_path("../../in_production_content/enhanced_videos"))
    DEFAULT_AUDIO_DIR = Path(get_mp3_paths("../../in_production_content/enhanced_audios"))
    DEFAULT_OUTPUT_DIR = Path("../../Post_production_content/mixed_video")
    
    def __init__(self):
        """Initialize mixer"""
        # Create default directories if they don't exist
        self.DEFAULT_VIDEO_DIR.mkdir(parents=True, exist_ok=True)
        self.DEFAULT_AUDIO_DIR.mkdir(parents=True, exist_ok=True)
...
Examples:
  ...
  python mixer.py -v video.mp4 -a audio.mp3 -o output.mp4
  
  # Mix new audio with original audio
  python mixer.py -v video.mp4 -a music.mp3 -o output.mp4 --mode mix
  ...
  python mixer.py -v video.mp4 -a audio.wav -o output.mp4 \\
      --audio-codec flac
...
Default Paths (modify in script):
  Videos: ../../videos/Pre_production_content
  Audio:  ../../audio/voiceovers
  Output: ../../videos/mixed_content
        """
    )
    ...
    parser.add_argument(
        '--video-dir',
        help='Video directory for batch mode (default: default video dir)'
    )
    
    parser.add_argument(
        '--audio-dir',
        help='Audio directory for batch mode (default: default audio dir)'
    )
    ...
    # Batch mode
    if args.batch:
        video_dir = args.video_dir or str(mixer.DEFAULT_VIDEO_DIR)
        audio_dir = args.audio_dir or str(mixer.DEFAULT_AUDIO_DIR)
        output_dir = args.output or str(mixer.DEFAULT_OUTPUT_DIR)
        
        if not Path(video_dir).exists() or not Path(audio_dir).exists():
            print("Error: Video or audio directory not found", file=sys.stderr)
            sys.exit(1)
        ...
        for video_file in video_files:
            ...
            audio_file = audio_path / f"{video_file.stem}.mp3"
            if not audio_file.exists():
                audio_file = audio_path / f"{video_file.stem}.wav"
            if not audio_file.exists():
                audio_file = audio_path / f"{video_file.stem}.m4a"
