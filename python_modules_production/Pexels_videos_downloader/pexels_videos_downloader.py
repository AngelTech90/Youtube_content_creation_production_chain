...
class PexelsVideoDownloader:
    ...
    DEFAULT_INPUT_DIR = Path("../../Core_data_workflow/data_sets/")
    DEFAULT_OUTPUT_DIR = Path("../../in_production_content/complementary_videos")
    
    def __init__(self, pexels_api_key, gemini_api_key, pkl_file_path=None):
        """
        ...
        Args:
            ...
            pkl_file_path (str): Path to the pickle file with timestamps (uses default if not specified)
        """
        ...
        if pkl_file_path:
            ...
            # Find first pkl file in default directory
            pkl_file = get_pkl_paths(str(self.DEFAULT_INPUT_DIR))
            if pkl_file:
                self.pkl_file_path = Path(pkl_file)
                print(f"Using pickle file: {self.pkl_file_path.name}")
            else:
                raise FileNotFoundError(
                    f"No .pkl files found in {self.DEFAULT_INPUT_DIR}"
                )
...
And you will use this general transcription wit time stamps as reference for get the search queries:
...
def main():
    ...
    # Run the downloader
    try:
        downloader = PexelsVideoDownloader(pexels_api_key, gemini_api_key)
        success = downloader.run_full_pipeline()
    except FileNotFoundError as e:
        print(f"Error: {e}")
        print(f"Usage: python pexels_videos_downloader.py")
        print(f"\nDefault pickle directory: ../../Core_data_workflow/data_sets/")
        sys.exit(1)
