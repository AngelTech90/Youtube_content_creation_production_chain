...
class MediaSeparator:
    ...
    DEFAULT_VIDEO_OUTPUT_DIR = Path("../../Pre_production_content/videos_separated")  # FIXED: Different from input!
    DEFAULT_AUDIO_OUTPUT_DIR = Path("../../Pre_production_content/audios")
    
    def __init__(self, video_dir: Optional[str] = None, audio_dir: Optional[str] = None):
        """
        Initialize separator with output directories
        
        Args:
            video_dir: Directory for video-only files (uses default if not specified)
            audio_dir: Directory for audio-only files (uses default if not specified)
        """
        # Create default input directory
        ...
        if video_dir:
            ...
            self.video_dir = self.DEFAULT_VIDEO_OUTPUT_DIR
        
        if audio_dir:
            self.audio_dir = Path(audio_dir)
        else:
            self.audio_dir = self.DEFAULT_AUDIO_OUTPUT_DIR
        ...
        print(f"Input directory:  {self.DEFAULT_INPUT_DIR}/")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Single file from default directory (default: MP4 video + MP3 audio)
  python video_and_separator.py
  
  # Single file with specific path
  python video_and_separator.py input.mp4
  
  # Multiple files
  python video_and_separator.py video1.mp4 video2.mkv video3.avi
  
  # Custom output directories
  python video_and_separator.py input.mp4 --video-dir my_videos --audio-dir my_audio
  
  # Custom output formats
  python video_and_separator.py input.mp4 --video-format mkv --audio-format wav
  
  # Batch process directory
  python video_and_separator.py *.mp4 --video-dir videos --audio-dir audio

Default Paths (modify in script):
  Input:  ../../Pre_production_content/videos
  Videos: ../../Pre_production_content/videos_separated
  Audio:  ../../Pre_production_content/audios

Output:
  Videos: Saved to videos_separated/ directory (no audio)
  Audio: Saved to audios/ directory (no video)
        """
    )
    ...
    parser.add_argument(
        'input',
        nargs='*',
        help='Input video file(s) (optional, uses default directory if not specified)'
    )
    
    parser.add_argument(
        '--video-dir',
        help='Output directory for video files (uses default if not specified)'
    )
    
    parser.add_argument(
        '--audio-dir',
        help='Output directory for audio files (uses default if not specified)'
    )
    ...
    if args.input:
        ...
        for ext in video_extensions:
            input_files.extend([str(f) for f in separator.DEFAULT_INPUT_DIR.glob(ext)])
        
        if not input_files:
            print(f"Error: No video files found in {separator.DEFAULT_INPUT_DIR}", file=sys.stderr)
            print("Please provide input files or place videos in the default input directory", file=sys.stderr)
            sys.exit(1)
