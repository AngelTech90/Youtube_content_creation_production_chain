...
Respond with ONLY the number (1-{len(titles)}) of the best title. No explanation, just the number."""
    ...
    def process(self, transcription_path: Optional[str] = None, output_path: Optional[str] = None, num_titles: int = 5):
        ...
        Args:
            ...
            output_path: Path to save title.txt (uses default if not specified)
            num_titles: Number of titles to generate
        """
        print(f"\n{'='*60}")
        ...
        if not output_path:
            output_path = self.DEFAULT_OUTPUT_DIR / "title.txt"
        else:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Generate title from first transcription in default directory
  python youtube_title_generator.py
  
  # Generate title from specific transcription file
  python youtube_title_generator.py --transcription transcription.txt
  
  # Generate title with custom output path
...
Default Paths (modify in script):
  ...
  Output:         ../../in_production_content/titles
...
Environment:
  GEMINI_API_KEY    Your Google Gemini API key
    ...
    parser.add_argument(
        '-o', '--output',
        help='Output file name (default: title.txt in output directory)'
    )
