...
class VideoIntegrator:
    ...
    def __init__(self, gemini_api_key, original_video=None, pkl_file=None, complementary_dir=None):
        ...
        Args:
            ...
            original_video (str): Path to original video file (uses default if not specified)
            pkl_file (str): Path to timestamps pickle file (uses default if not specified)
            complementary_dir (str): Directory with complementary videos (uses default if not specified)
        """
...
Return the ideal durations list now:"""
    ...
    def trim_videos_with_ffmpeg(self):
        ...
        for idx, (video_name, ideal_dur) in enumerate(
            ...
            try:
                # FFmpeg command to trim video (copy codec for speed)
                cmd = [
                    "ffmpeg",
                    "-i", str(video_path),
                    "-t", str(ideal_dur),
        ...
        try:
            ...
            if filter_complex:
                cmd.extend([
                    ...
                    "-map", final_output,  # keep the brackets!
                    "-map", "0:a"
                ])
...
def main():
    ...
    try:
        ...
        print(f"Error: {e}")
        print("\nUsage: python pexels_videos_indexer.py [ORIGINAL_VIDEO] [PKL_FILE] [COMPLEMENTARY_DIR]")
        print("\nExample:")
        print("  python pexels_videos_indexer.py")
        print("  python pexels_videos_indexer.py my_video.mp4 timestamps.pkl complementary_videos/")
        print("\nDefault Paths (modify in script):")
        print(f"  Original Video: {VideoIntegrator.DEFAULT_INITIAL_VIDEO_DIR}")
        print(f"  Pickle File:    {VideoIntegrator.DEFAULT_PKL_DICT}")
        print(f"  Complementary:  {VideoIntegrator.DEFAULT_COMPLEMENTARY_VIDEOS}")
        sys.exit(1)
